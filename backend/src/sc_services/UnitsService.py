...
class UnitsService(ServiceAbstract):
    ...
    def get_units_all(database):
        return database.session.query(UnitsTable).all()


    def get_unit_by_name(database, name):
        unit = database.session.query(UnitsTable).filter_by(name=name).first()
        if not unit:
            return None
        return unit
    ...
    def get_unit_by_id(database, unit_id):
        unit = database.session.query(UnitsTable).filter_by(id=unit_id).first()
        if not unit:
            return None
        return unit
    ...
    def get_units_list(database):
        return [unit.name for unit in database.query(UnitsTable).all()]
    ...
    def get_available_units_id(database, root_unit_name):
        ...
        for id in next_ids:
            next_ids.extend([unit.id for unit in database.session.query(UnitsTable).filter_by(root_id=id).all()])
        return next_ids
